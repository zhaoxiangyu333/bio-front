          ...
          <ul class="address-item-list clearfix">
           ...
           <span v-if="id === item.id" class="pa">
             ...
             <path
              ...
              <p class="street-name ellipsis">收货地址: {{item.remark}}</p>
...
<script>
  import {generalOrder, getCartList, addressList, addressUpdate, addressAdd, addressDel, productDet} from '/api/goods'
  ...
  export default {
    ...
    methods: {
      _getCartList () {
        getCartList({token: getStore('token')}).then(res => {
      ...
      payment () {
        // 需要拿到地址id
        generalOrder({token: getStore('token'), addressId: this.id}).then(res => {
          if (res.flag) {
            this.$router.push({
              path: '/order/payment',
              query: {
