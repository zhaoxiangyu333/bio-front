    ...
    <div>
      <header class="w">
        <div class="w-box">
          <div class="nav-logo">
            <h1>
              <router-link to="/" title="">BioShop</router-link>
            </h1>
          </div>
          <div class="right-box">
            <div class="nav-list">
                  ...
                  <div class="nav-user-list">
                    <ul>
                      ...
                      <li class="nav-user-avatar">
                        <div>
                          <span class="avatar" :style="{backgroundImage:'url('+userInfo.img+')'}">
                          </span>
                        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      delGoods (productId) {
        if (this.login) { // 登陆了
          delCart({token: getStore('token'), productId}).then(res => {
      ...
      // 退出登陆
      _loginOut () {
        loginOut({token: getStore('token')}).then(res => {
          removeStore('token')
